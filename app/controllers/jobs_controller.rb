class JobsController < ApplicationController
 ...
 #AI
    ...
    chaptgpt_response = client.chat(parameters: {
      model: "gpt-3.5-turbo",
      messages: [{ role: "user", content: "I will give you two array-like objects, the first containing information for a candidate/s profile and the second containing information for a job posting. I want you to cross-match the information on the candidate/s skills focusing on key-words (particularly #{address} , #{summary}, #{tech_interest}, #{tech_languages} and #{preferred_companies} and the information on the job listing (particularly #{title}, #{job_description}, #{soft_skills}, #{programming_languages}, #{location}) and return a ranking from highest match to poorest job match compatibility. Ensure you also consider if any word in the job listing's information matches a word in the candidate summary."}]
    })
